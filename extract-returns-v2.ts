...
function mergeExtractedData(extractedData: any[], domain: string): any {
  ...
  const mergedData = { ...extractedData[0].data };
  
  // Merge additional data from other pages
  for (let i = 1; i < extractedData.length; i++) {
    const pageData = extractedData[i].data;
    const pageType = extractedData[i].page;
    
    console.log(`[${domain}] Merging data from ${pageType} page...`);
  ...
  console.log(`[${domain}] Data merging completed`);
  return mergedData;
}
...
async function processDomain(domain: string) {
  ...
  try {
    ...
    const variables = { website: domain, country: 'Hungary' };
    ...
    const navigationResult = await executeNavigationStrategy(page, domain, returnSchema);
    ...
    if (navigationResult.success && navigationResult.data.length > 0) {
      ...
      if (mergedData) {
        console.log(`[${domain}] Successfully merged data from ${navigationResult.data.length} pages`);
        return mergedData;
      }
    }
        ...
        for (const additionalPrompt of additionalPrompts) {
          try {
            ...
            if (obsResult && Array.isArray(obsResult) && obsResult.length > 0 && obsResult[0]?.selector) {
              ...
              await page.act({ description: "Scroll through the entire page to see all information" });
              await page.waitForTimeout(2000);
              ...
              await page.goBack();
              await page.waitForTimeout(3000);
              await page.act({ description: "Scroll to the bottom of the page to see footer links" });
              await page.waitForTimeout(2000);
    ...
    await page.act({ description: "Scroll through the entire page from top to bottom to see all content" });
    await page.waitForTimeout(3000);
    
    const homepageData = await page.extract({
      instruction: `Extract all return information including return periods, methods, costs, conditions, and procedures for returning products to ${domain}. Look carefully at the footer, navigation, and all sections of the page. If no specific return information is found, indicate this clearly.`,
      schema: returnSchema
    });
