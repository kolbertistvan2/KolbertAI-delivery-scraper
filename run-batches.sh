...
RESULT_DIR="$(pwd)/result-delivery"
MAX_RETRIES=3
...
total=${#domains[@]}
echo "Processing $total domains with batch size $BATCH_SIZE"
...
for ((i=0; i<total; i+=BATCH_SIZE)); do
  ...
  for domain in "${batch[@]}"; do
    result_file="$RESULT_DIR/shipping-info-$domain"
    ...
    (
      ...
      while [ $attempt -le $MAX_RETRIES ]; do
        ...
        docker run --rm --env-file .env -v "$RESULT_DIR:/app/result-delivery" $IMAGE "$domain"
        exit_code=$?
